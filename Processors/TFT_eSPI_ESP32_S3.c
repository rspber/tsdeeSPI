...
***************************************************************************************/
void TFT_eeSPI::begin_SDA_Read(void)
{
...
***************************************************************************************/
void TFT_eeSPI::end_SDA_Read(void)
{
...
***************************************************************************************/
void TFT_eeSPI::busDir(uint32_t mask, uint8_t mode)
{
...
***************************************************************************************/
void TFT_eeSPI::gpioMode(uint8_t gpio, uint8_t mode)
{
  pinMode(gpio, mode);
...
***************************************************************************************/
void TFT_eeSPI::pushSwapBytePixels(const void* data_in, uint32_t len){
...
void TFT_eeSPI::pushImageDMA16(int32_t x, int32_t y, int32_t w, int32_t h, uint16_t const* image)
{
...
void TFT_eeSPI::pushImageDMA16(clip_t& clip, int32_t x0, int32_t y0, int32_t w, int32_t h, bool swapBytes, uint16_t* image, uint16_t* buffer)
{
  ...
  // of 32768 pixels maximum. (equivalent to an area of ~320 x 100 pixels)
  while(len>0x4000) { // Transfer 16-bit pixels in blocks if len*2 over 65536 bytes
    pushPixels16(buffer, 0x400, false);
    len -= 0x400; buffer+= 0x400; // Arbitrarily send 1K pixel blocks (2Kbytes)
  }
  esp_err_t ret;
  static spi_transaction_t trans;
...
***************************************************************************************/
bool TFT_eeSPI::initDMA(bool ctrl_cs)
{
